...
const PO_VH_ITEMS: ValueHelpItem[] = [
  { id: "PO2001", description: "Steel Rod Purchase Order", plant: "1000", vendor: "Vendor A", status: "Open" },
  { id: "PO2002", description: "Copper Wire Purchase Order", plant: "2000", vendor: "Vendor B", status: "Pending" },
];
...
const INITIAL_DOCS = [
  { fileName: "PO2001_Order.pdf", docNumber: "PO2001", uploadDate: "2025-02-18", uploadedBy: "Jane Smith", size: "0.6 MB" },
];
...
export function POModule() {
  ...
  const filteredPO = ALL_PO_DATA.filter((row) => {
    if (filters.docNumber && row.poNumber !== filters.docNumber) return false;
  ...
  const processFile = (name: string) => {
    ...
    setTimeout(() => {
      ...
      setOcr({
        ...
        matching: !!uploadDocNumber && PO_VH_ITEMS.some((i) => i.id === uploadDocNumber),
      });
    }, 1200);
  };
  ...
  const handleUpload = () => {
    ...
    // PO/GRN/Invoice: single document — replace existing
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <FilterBar docType="PO" valueHelpItems={PO_VH_ITEMS} onSearch={(f) => setFilters(f)} />
      ...
      <div className="flex-1 overflow-auto p-4 flex flex-col gap-4">
        ...
        <div className="border" style={{ backgroundColor: "#ffffff", borderColor: "#d9d9d9", borderRadius: "2px" }}>
          <div className="px-4 py-2 border-b" style={{ backgroundColor: "#f5f5f5", borderColor: "#d9d9d9" }}>
            <span style={{ fontSize: "12px", fontWeight: "600", color: "#32363a" }}>PO Items ({filteredPO.length})</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "#f5f5f5", borderBottom: "1px solid #d9d9d9" }}>
                  <th style={{ width: "32px", padding: "6px 8px", borderRight: "1px solid #e5e5e5" }}>
                    <input type="checkbox" onChange={(e) => { if (e.target.checked) setSelectedRows(filteredPO.map((r) => r.poNumber + r.item)); else setSelectedRows([]); }} />
                  </th>
                  {["Document Type", "Purchase Organization", "Purchase Group", "Company Code", "Vendor", "Item Number", "Material", "Quantity", "Net Price", "Delivery Date", "Plant", "Storage Location"].map((col) => (
              ...
              <tbody>
                {filteredPO.length === 0 ? (
                ...
                ) : filteredPO.map((row, i) => {
                  const rowKey = row.poNumber + row.item;
                  ...
                  return (
                    <tr key={rowKey} onClick={() => toggleRow(rowKey)}
                      ...
                      <td style={{ padding: "5px 10px", fontSize: "12px", color: "#32363a", borderRight: "1px solid #e5e5e5" }}>{row.purchOrg}</td>
                      <td style={{ padding: "5px 10px", fontSize: "12px", color: "#32363a", borderRight: "1px solid #e5e5e5" }}>{row.purchGroup}</td>
                      <td style={{ padding: "5px 10px", fontSize: "12px", color: "#32363a", borderRight: "1px solid #e5e5e5" }}>{row.companyCode}</td>
                      <td style={{ padding: "5px 10px", fontSize: "12px", color: "#32363a", borderRight: "1px solid #e5e5e5" }}>{row.vendor}</td>
                      ...
                      <td style={{ padding: "5px 10px", fontSize: "12px", color: "#32363a", borderRight: "1px solid #e5e5e5", textAlign: "right" }}>{row.netPrice.toLocaleString()}</td>
            ...
            {ocr && (
              <div className="border flex-shrink-0" style={{ width: "240px", backgroundColor: "#ffffff", borderColor: "#d9d9d9", borderRadius: "2px" }}>
                ...
                <div className="p-3 flex flex-col gap-2">
                  {[{ label: "Document Type", value: ocr.docType }, { label: "Document Number Detected", value: ocr.docNumber }, { label: "PO Matching", value: ocr.matching }].map((item) => (
                  ...
                  <div className="mt-1 p-2 border" style={{ borderColor: "#d9d9d9", backgroundColor: "#f5f5f5", borderRadius: "2px" }}>
                    {ocr.docType && ocr.docNumber && ocr.matching
                      ...
                      : <div className="flex items-start gap-1"><AlertCircle size={12} color="#E9730C" style={{ marginTop: "1px" }} /><span style={{ fontSize: "11px", color: "#E9730C" }}>Some checks failed. Review before uploading.</span></div>}
                  </div>
                </div>
              </div>
            )}
